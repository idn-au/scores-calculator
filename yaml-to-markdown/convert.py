    ...
    for req in contents:
        render_str += f"""
            <tr>
                ...
                {f"<td {'colspan="2"' if not req.get('conditions') else ''}><p>Query</p><pre>{req['query']}</pre></td>" if req.get("query") else ""}
                {f"<td {'colspan="2"' if not req.get('query') else ''}><p>Conditions</p><ul>{''.join([f'<li>{cond["title"]}</li>' for cond in req['conditions']])}</ul></td>" if req.get("conditions") else ""}
            </tr>
        """
    render_str += "</table>"
    return render_str
...
def main():
    ...
    print(render_str)
    with open(PROJECT_DIR.parent / "test2.html", "w") as f:
        f.write(render_str)
